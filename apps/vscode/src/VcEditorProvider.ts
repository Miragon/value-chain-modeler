 ...
 */
export class VcEditorProvider implements vscode.CustomTextEditorProvider {
  public static readonly viewType = 'valueChain.diagramEditor';

  public static register(context: vscode.ExtensionContext): vscode.Disposable {
    return vscode.window.registerCustomEditorProvider(
      VcEditorProvider.viewType,
      new VcEditorProvider(context),
      {
  ...
  public async resolveCustomTextEditor(
    ...
    // onDidChangeTextDocument) and NOT reset the canvas. A SET (not a single value): for
    // edits A,B in quick succession, EVERY echo must be suppressed, regardless of the order
    // in which the change events arrive.
  ...
  private async replaceWholeDocument(
    ...
    // Register first (synchronously, before applyEdit), so the resulting change event is
    // reliably recognized as an echo. Only add when a write actually happens (not a no-op
    // above) -> no orphaned set entries.
